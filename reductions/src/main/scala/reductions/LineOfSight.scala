...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    def myMax(input: Array[Float], from: Int, until: Int, currentMax: Float): Float = {
      if (from < until && from > 0) {
        myMax(input, from + 1, until, max(currentMax, (input(from) / from)))
      } else {
        currentMax
      }
    }
    myMax(input, from, until, 0f)
  }
  ...
  def upsweep(input: Array[Float], from: Int, end: Int,
    ...
    if (end - from <= threshold) {
      ...
      val (val1, val2) = parallel(upsweep(input, from, mid, threshold), upsweep(input, mid, end, threshold))
      Node(val1, val2)
    }
  }
