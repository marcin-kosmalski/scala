...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("2countChange should work for multi-coins") {
    def check(money: Int, coins: List[Int], expected: Int) =
      assert(parCountChange(money, coins, moneyThreshold(10)) == expected,
        s"countChange($money, $coins) should be $expected")
  ...
  test("3countChange should work for multi-coins") {
    def check(money: Int, coins: List[Int], expected: Int) =
   ...
   test("4countChange should work for multi-coins") {
    def check(money: Int, coins: List[Int], expected: Int) =
      assert(parCountChange(money, coins, combinedThreshold(10,List(1,2))) == expected,
        s"countChange($money, $coins) should be $expected")
