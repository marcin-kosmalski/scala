 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum list with zero elements"){
    assert(sum(List())===0)
  }
  
  test("sum negativ values"){
    assert(sum(List(-3,1))==(-2))
  }
  
  test("max negativ values"){
    assert(max(List(-3,-2,0))==0)
  }
  
  test("max list with zero elements"){
