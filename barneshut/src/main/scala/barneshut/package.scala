...
package object barneshut {
  ...
  case class Fork(
    ...
    def isWithin(q: Quad, b: Body): Boolean = {
      val half = q.size / 2;
      q.centerX - half <= b.x && b.x <= q.centerX + half &&
        q.centerY - half <= b.y && b.y <= q.centerY + half
    }
  ...
  case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: Seq[Body])
    ...
    val (mass, massX, massY) = (leafMass: Float, if (leafMass == 0) 0 else
      (bodies.foldLeft(0f)((sum, body) => body.x * body.mass + sum)) / leafMass: Float, if (leafMass == 0) 0 else
      bodies.foldLeft(0f)((sum, body) => body.y * body.mass + sum) / leafMass: Float)
    val total: Int = bodies.size
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        val newSize = size / 2
        val quarterLen = newSize / 2;
        var fork = Fork(Empty(centerX - quarterLen, centerY - quarterLen, newSize), Empty(centerX + quarterLen, centerY - quarterLen, newSize),
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      ...
      bx = Math.abs(boundaries.minX - bx)
      by = Math.abs(boundaries.minY - by)
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      ...
      for (i <- 0 until matrix.length) {
        sm.matrix.update(i,matrix(i).combine(that.matrix(i)))
      }
      sm
    }
